...
export default function CreateModuleButton() {
  ...
  const [form, setForm]       = useState({
    ...
    title: '', description: '', is_visible: true,
  })
  ...
  return (
    <>
      <button onClick={() => setOpen(true)} className="btn-primary"><Plus size={16}/> Nieuwe module</button>
      {open && (
          ...
          <div className="relative bg-white rounded-2xl shadow-modal w-full max-w-md animate-slide-up">
            <div className="flex items-center justify-between p-6 border-b border-border">
              <div className="flex items-center gap-2">
                <div className="w-8 h-8 bg-primary-50 rounded-lg flex items-center justify-center"><BookOpen size={16} className="text-primary-600"/></div>
                <h2 className="font-semibold text-gray-900">Nieuwe lesmodule</h2>
              </div>
            ...
            <div className="p-6 space-y-4">
              ...
              <div><label className="label">Naam *</label>
                <input type="text" value={form.title} onChange={e => setForm(p => ({ ...p, title: e.target.value }))} placeholder="Bijv. Week 3 — Tajweed" className="input"/>
              </div>
              <div><label className="label">Beschrijving <span className="text-gray-400">(optioneel)</span></label>
                <textarea rows={2} value={form.description} onChange={e => setForm(p => ({ ...p, description: e.target.value }))} placeholder="Korte beschrijving…" className="input resize-none"/>
              </div>
            ...
            <div className="flex gap-3 p-6 border-t border-border">
              ...
              <button onClick={handleCreate} disabled={loading || !form.class_id || !form.title.trim()} className="btn-primary flex-1 justify-center">
                {loading ? <><Loader2 size={15} className="animate-spin"/> Aanmaken…</> : 'Module aanmaken'}
              </button>
            </div>
          </div>
        </div>
